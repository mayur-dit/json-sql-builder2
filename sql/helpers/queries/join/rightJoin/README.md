# rightJoin Helper
Specifies the `RIGHT JOIN` operator for the `FROM` clause.
...
Usage of `rightJoin` as **Object** with the following Syntax:
...
**SQL-Definition:**
```javascript
RIGHT JOIN{ LATERAL[$lateral]}-->(PostgreSQL){ [$table]}{ [$select]} AS <key-ident>{ ON [$on]}{ USING [$using]}
```
